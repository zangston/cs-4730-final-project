...
{
    public GameObject plasmaPrefab; // Assign Plasma prefab
    ...
    void Start()
    {
        ...
        spawnInterval = 1.0f;
        // InvokeRepeating("SpawnPlasmaRandom", 2.0f, spawnInterval);
        InvokeRepeating("SpawnPlasmaAtPlayer", 2.0f, spawnInterval);
    }

    // Given X and Y positioning relative to spawner location (0, 0), spawn a plasma bolt with a given direction
    void SpawnPlasma(float spawnX, float spawnY, float spawnRotation)
    {
        ...
        Instantiate(plasmaPrefab, spawnPosition, Quaternion.Euler(0, 0, spawnRotation));
    }

    // Spawn one plasma bolt at a random edge of the level in a random direction
    void SpawnPlasmaRandom()
    {
